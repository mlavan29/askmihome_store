    ...
    <script>
        ...
        async function getTotalEarnings() {
            ...
            await db.collection('restaurant_orders').where('vendorID', "==", vendorId).where('status', 'in', ["Order Completed"]).get().then(async function (orderSnapshots) {
                ...
                tax = 0;
                if (orderData.taxSetting != undefined && $.isArray(orderData.taxSetting)) {
					for (let i = 0; i < orderData.taxSetting.length; i++) {
						let taxData = orderData.taxSetting[i];
                ...
                if (orderData.tip_amount != undefined && orderData.tip_amount != "" && orderData.tip_amount > 0) {
                    price = price + parseFloat(orderData.tip_amount);
                }
                    ...
                    if (orderData.adminCommission != undefined && orderData.adminCommissionType != undefined && orderData.adminCommission > 0 && price > 0) {
                        ...
                        if (orderData.adminCommissionType == "Percent") {
                            commission = (total_item_price * parseFloat(orderData.adminCommission)) / 100;
